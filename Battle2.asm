...
BFYpos  dw 104
PlayerTurn db 1 ; 0 first player , 1 second player  ; should change after every hit is done 
...
Player2: ; here is refered as the player with cannon under Player1 Grid 
...
KeepTaking1:mov ah,1                ; i keep looping untill i take any kind of Key pressed 
		int 16h
		JZ KeepTaking1 
		mov ah,0
		int 16h
...
MyTurn:
		mov ah,1
		int 16h	
CheckMe:JNZ TakeValue  ; take the ypos in cx and send it to the game logic so what ever the calculation u want to perform
		call FillBarProc
		mov ax,BFYpos
		cmp ax,56
		JNZ ChoosePower1
